...
k = Math.sqrt(json.nodes.length / (width * height));
...
function mouseout(d) {
  d3.select(this).select("circle").transition()
      .duration(100)
      .style("stroke-width",function (d){var sw="1.0px"; if (d.index==0) sw="1.4px"; return sw;})
      .attr("r", circleRadius);
}
...
function wrap(text, width) {
  text.each(function() {
    var text = d3.select(this),
      ...
      tspan = text.text(null).append("tspan").attr("x", 0).attr("y", y).attr("dy", dy + "em").attr("text-anchor", "middle");
    var lineNum = 0;
